...
function ImGuiInterface:renderAdvancedSettings()
    if imgui.CollapsingHeader('Advanced Settings') then
        ...
        -- Roll with Bust setting
        local rollwithbustValue = { self.settings.rollwithbust };
        if imgui.Checkbox('Roll 2 When Busted', rollwithbustValue) then
            self.settings.rollwithbust = rollwithbustValue[1];
            self.libSettings.save();
        end
        imgui.SameLine();
        imgui.TextColored({ 0.7, 0.7, 0.7, 1.0 }, '(?)');
        if imgui.IsItemHovered() then
            imgui.SetTooltip('Allow Roll 2 even when busted. Party still benefits from both rolls.\nWhen disabled, prioritizes bust recovery over Roll 2.');
        end

        -- Smart Snake Eye setting
        local smartsnakeeyeValue = { self.settings.smartsnakeeye };
        if imgui.Checkbox('Smart Snake Eye Optimization', smartsnakeeyeValue) then
            self.settings.smartsnakeeye = smartsnakeeyeValue[1];
            self.libSettings.save();
        end
        imgui.SameLine();
        imgui.TextColored({ 0.7, 0.7, 0.7, 1.0 }, '(?)');
